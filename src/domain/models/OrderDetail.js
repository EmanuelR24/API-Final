            ...
            <tbody>
              {order.details && order.details.length > 0 ? (
                order.details.map(d => {
                  ...
                  return (
                    <tr key={d._id || d.productoId} className="hover:bg-opacity-50 transition-all duration-300">
                      <td className="p-2 text-soft-white">{productName}</td>
                      <td className="p-2 text-soft-white">{d.cantidad || 'N/A'}</td>
                      <td className="p-2 text-soft-white">{d.precioUnitario || 'N/A'}</td>
                      <td className="p-2 text-soft-white">{d.subtotal || 'N/A'}</td>
                    </tr>
                  );
                })
              ) : (
